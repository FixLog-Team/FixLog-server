...
JIRA 이슈 티켓을 조회하고 변경된 코드를 분석하여 TDD 기반 테스트 시나리오를 설계합니다.

- JIRA 이슈 번호 자동 추출 (브랜치명 기반) 및 티켓 정보 조회 (Atlassian MCP 연동)
- 단위 / 슬라이스 / 통합 테스트 계층별 시나리오 분리
- 실패·예외 케이스 중심 설계 (인증 없음, 입력 검증 실패, 리소스 없음, 권한 없음 등)
- Given-When-Then 형식으로 테스트 메서드명과 시나리오 제시
- 선택적으로 테스트 코드 스켈레톤 파일 생성
